...
function connectionIcon(kind, signalStrength, connectivity) {
  // portal-pop: locked wifi icon when NM says we're behind a captive portal or have no route out.
  if (kind === "wifi" && (connectivity === "portal" || connectivity === "limited" || connectivity === "none")) return "󰤪"
...
// with it either).
// portal-pop: probe a plain-http URL, open whatever the portal redirects to (what phones do).
// No redirect = no portal; say so instead of opening the bait page.
var openPortalScript =
  "u=$(curl -s -m 5 -o /dev/null -w '%{redirect_url}' http://ping.archlinux.org);" +
  " case \"$u\" in" +
  "   http://*|https://*) xdg-open \"$u\" ;;" +
  "   *) omarchy-notification-send --app-name Wi-Fi -g 󰤬 -t 5000 'No sign-in page' 'This network is already online.' ;;" +
  " esac"
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    openPortalScript: openPortalScript,
